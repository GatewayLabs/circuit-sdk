...
use garble_lang::token::MetaInfo;
...
mod tests {
    ...
    use crate::circuit::unsigned_as_wires;
    use crate::circuit::CircuitBuilder;

    #[test]
    fn test_xor_circuit_8bit() {
        // Initialize the circuit builder with two 8-bit inputs (for two parties).
        let mut builder = CircuitBuilder::new(vec![8, 8]);

        // Add XOR gates for each corresponding bit.
        let mut xor_gates = vec![];
        for i in 0..8 {
            let wire_a = i;
            let wire_b = i + 8; // Offset for the second party's input wires.
            let xor_gate = builder.push_xor(wire_a, wire_b);
            xor_gates.push(xor_gate);
        }

        // Build the circuit with the XOR output gates.
        let circuit = builder.build(xor_gates);

        // Define two 8-bit inputs.
        let input_a = vec![true, false, true, false, true, false, true, false]; // Binary: 10101010
        let input_b = vec![false, true, false, true, false, true, false, true]; // Binary: 01010101
        ...
        // Expected XOR result: 10101010 ^ 01010101 = 11111111 (All bits should be 1).
        let expected = vec![true, true, true, true, true, true, true, true]; // Binary: 11111111
    ...
    #[test]
    fn test_xor_circuit_1bit() {
        let mut builder = CircuitBuilder::new(vec![1, 1]);
        let xor = builder.push_xor(0, 1);
        let circuit = builder.build(vec![xor]);

        let x = vec![true];
        let y = vec![false];
        let result = circuit.eval(&[x, y]);
    ...
    #[test]
    fn test_xor_circuit() {
        let mut builder = CircuitBuilder::new(vec![4, 4]);
        let x = unsigned_as_wires(5, 4);
        let y = unsigned_as_wires(10, 4);

        let xor = builder.push_xor(x[0], y[0]);
        let circuit = builder.build(vec![xor]);
    ...
    fn test_push_addition_circuit() {
        ...
        let (sum, carry, _) = builder.push_addition_circuit(&x, &y);
        let circuit = builder.build(vec![sum[3]]);
